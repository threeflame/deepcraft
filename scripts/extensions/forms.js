...
class ChestFormData {
    ...
    // ★変更: 第2引数に showInventory を追加 (デフォルト true)
    constructor(size = 'small', showInventory = true) {
        ...
        this.showInventory = showInventory; // 設定を保存
    }
    title(text) {
    ...
    show(player) {
        ...
        // ★変更: inventory_enabled が false または 個別の showInventory が false なら処理をスキップ
        if (!inventory_enabled || !this.showInventory) return form.show(player);
